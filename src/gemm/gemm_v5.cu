...
__global__ void mysgemm_v5(int M,int N,int K,float alpha,float *A,float beta,float *B,float *C){
...
int main()
{
    ...
    for (int i = 0; i < warmup; ++i) {
        mysgemm_v5<BM,BN,BK,TM,TN><<<grid, block>>>(
            M, N, K,
            alpha, A,
            beta, B, C
        );
