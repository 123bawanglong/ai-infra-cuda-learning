...
__global__ void mysgemm_v2(int M,int N,int K,float alpha,float *A,float beta,float *B,float *C){
    ...
    for(int bk=0;bk<K;bk+=BK){
        ...
        #pragma unroll
        for(int k=0;k<BK;k++){
            sum+=As[OFFSET(ty,k,BK)]*Bs[OFFSET(k,tx,BN)];
        }
        __syncthreads();
    }
    if(row>=M||col>=N)return;
    int index=OFFSET(row,col,N);
    C[index]=(beta==0.0f)?alpha*sum:alpha*sum+beta*C[index];
}
int main()
{
    const int M = 4096;
    ...
    for (int i = 0; i < warmup; ++i) {
        mysgemm_v2<BM,BN,BK><<<grid, block>>>(
            M, N, K,
            alpha, A,
            beta, B, C
        );
