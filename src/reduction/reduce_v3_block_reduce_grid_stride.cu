...
__global__ void reduceBlockGridStride(const float* input, float* output,int n) {
    float localSum = 0.0f;
...
int main() {
    ...
    reduceBlockGridStride<<<blockCount, BLOCK_SIZE>>>(deviceInput, devicePartialSums, n);
    reduceBlockGridStride<<<1, BLOCK_SIZE>>>(devicePartialSums, deviceResult, blockCount);
    cudaEventRecord(end);
