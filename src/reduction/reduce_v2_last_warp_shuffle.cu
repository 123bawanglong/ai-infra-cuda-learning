...
__global__ void reduceLastWarpShuffle(const float* input, float* output,int n) {
    ...
    if (tid < 32) {
        float value = sharedData[tid] + sharedData[tid + 32];
...
int main() {
    ...
    cudaMalloc(reinterpret_cast<void**>(&devicePartialSums), sizeof(float) * blockCount);
    ...
    reduceLastWarpShuffle<<<blockCount, BLOCK_SIZE>>>( deviceInput, devicePartialSums, n);
    reduceLastWarpShuffle<<<1, BLOCK_SIZE>>>(devicePartialSums, deviceResult, blockCount);
    cudaEventRecord(end);
