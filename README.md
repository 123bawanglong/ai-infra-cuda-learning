...
# 设置为你的 GPU 架构；RTX 5080 使用 sm_120。
CUDA_ARCH=sm_120
...
将 FlashAttention kernel 编译为目标文件：
...
请为编译命令添加与你的 GPU 对应的架构参数，例如 `-arch=sm_89`。

## 快速正确性检查

```bash
# GEMM 基本运行检查（固定 4096 × 4096 输入，预期 C[0] = 4096）
build/gemm_v1
build/gemm_tensor
...
准备 input_320x4096.txt：第一行为 `320 4096`，后面按行优先顺序存放
320 × 4096 个浮点数，各数值用空白字符分隔。
...
比较 kernel 耗时、内存带宽、global memory 访问效率、shared memory 的 bank conflict、
occupancy、寄存器使用量和 warp 执行效率。

## 当前实现范围

- 算子主要使用 FP32；GEMM Tensor Core 版本使用 TF32 乘法和 FP32 累加。
- FlashAttention 实现单头计算，尚不包含因果掩码、批处理或混合精度。
- GEMM 实验部分保留已有结果；可复现的硬件、编译参数及代码版本对应关系仍需补充。
- 本仓库用于学习和实验，不作为生产级算子库的替代品。

## 简历项目描述参考

> 实现并优化 SGEMM、Reduction、Softmax、RMSNorm、Transpose、Histogram 和 FlashAttention 等 CUDA 算子。
> 运用 shared memory 分块、向量化访存、寄存器分块、warp shuffle、双缓冲与在线 Softmax，
> 配合正确性检查和 Nsight Compute 分析定位性能瓶颈。

## 后续完善方向

- 为每个算子补充自动化正确性测试。
- 在同一 NVIDIA GPU 上对比各阶段性能。
- 增加 FP16/BF16 Tensor Core 实现。
- 增加批处理、多头和因果 FlashAttention 版本。
- 与 cuBLAS 和框架基线对比。
